...
class Command(BaseCommand):
    ...
    def handle(self, *a, **o):
        ...
        df.columns = [c.strip().lower().lstrip("\ufeff") for c in df.columns] # conserta as colunas do df
        ...
        df = df.query("nome !='' and sobrenome != '' ") # consulta onde nome E sobrenome são vazios e gera um novo df SEM eles

        df = df.dropna(subset=["data_nascimento"]) # exclui a linha que não possuir data
